...
MM_PER_PIXEL = 0.1             # 像素当量设定值（mm/像素）；
                               # 运行期换算请走 part_model.mm_per_pixel()
                               # （优先级：新鲜标定 > 此设定值）
...
TEMPLATE_MARGIN_PX = 30      # 模板裁剪余量（px）：part_model.make_template
                             # 的裁剪半边长 = FLANGE_R_PX + 本值；同时参与
                             # 模板参数指纹与缓存尺寸校验（改这里即整体生效）
...
#   注：inspect 输出的 confidence 只作展示/复核参考，不判级
#  （曾设想"低于 CONF_MIN_OK 标记低置信"，该功能未实现，参数已删）。
